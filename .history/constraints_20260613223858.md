...
- **Mapeo flexible:** Resolver siempre los campos dinámicamente utilizando búsquedas tolerantes e insensibles a mayúsculas, minúsculas y acentos sobre los labels. Utilizar arrays de palabras clave alternativas (ej: `['id_alumno', 'dni', 'documento']`).
...
## 6. Persistencia del Campo Materias
- **Formato:** Se almacena como un string JSON directo en Moodle (ej: `[{"materia":"Álgebra I","nota":6}]`).
- **Tipo de campo:** Debe guardarse en un campo de tipo **Área de texto** (Text area) llamado `materias`, ya que los tipos de texto corto truncarán el JSON al superar el límite de caracteres.
